...
resource "aws_codebuild_project" "tf-scan" {
  name          = "tf-cicd-scan"
  description   = "Security Scan stage for terraform"
  service_role  = aws_iam_role.tf-codebuild-role.arn
 ...
 source {
     type   = "CODEPIPELINE"
     buildspec = file("buildspec/scan-buildspec.yml")
 }
}
